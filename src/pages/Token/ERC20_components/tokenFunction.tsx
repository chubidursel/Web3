...
//<<<<<<<<<<<<<<<<<<<<
  const hadleSupply = async()=>{
    try {
      ...
      setBalance(balance.toString())
        ...
        else {setError("Some mystic error")}
      }
      setLoaderBal(false)
  }
  ...
  const handleTransaction = async(event:any)=>{
    ...
    // else if(isNaN(amountTo) || (amountTo == "" ) || (amountTo == 0)){setError("Incorrect Amount")}
    try {
      ...
      <div className='bg-blue-100 rounded-2xl border-4 border-red-400 text-xl px-[15px]'>
       ...
       <div className='rounded-2xl border-2 border-red-400 px-[15px] p-2 m-2'>
          <h1 className='text-center font-bold'>Check your balance</h1>
            ...
            </div> : <button onClick={hadleSupply} className="font-bold ml-3 rounded-2xl border-2 border-red-400 px-[15px] hover:bg-red-400">Submit</button>}
        ...
        {loader ? 
            ...
            {/* {error ? <div className={error=="Transaction Success" ? "text-green-500 text-2xl font-bold" : "text-red-500 text-2xl font-bold"}>{error}</div> : null} */}
